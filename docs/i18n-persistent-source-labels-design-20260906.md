...
> 状态：**已落地（2026-09-14 核对）**——最终采纳 §7-B 方案：`lib/logic/source_tag.dart` 的 `SourceTag` 静态工具类（keyOf / localize / isAlbum / isOnline），非 §7-A 的 enum；`sourceLabelSnapshot` 默认值已 key 化（默认 `'main'`），`keyOf` 兼容历史中文值归一迁移；i18n key `source.*` 已全部存在（`strings_zh.g.dart`）。§1.1 的"现状"描述为拍板前快照。
...
> 决策依据：早前拍板「存储改稳定 key（新写入 + 存量迁移）」而非「仅展示时映射」。本文保留为该决策的设计记录。
