...
- **多块集群尺寸锁定**：已吸附拼合的多块集群（逻辑上永远无法放回托盘）拖拽全程锁定棋盘尺寸 `_zoom`，不再按光标 Y 坐标过渡缩小到托盘尺寸——消除"拖入托盘区域缩小、松手弹回"的误导手感；单块碎片保留上述进出托盘过渡缩放（放回托盘的视觉暗示）。
...
- **8 扇区环形全域发散**：将屏幕除中央棋盘安全区外的空间划分为 8 个方位扇区，四向交替（Round-Robin）注入槽位、`tabletopScatterStepRatio = 1.45 · pieceSize` 步长多行多列、基于关卡种子 `±8%` 自然抖动，棋盘居中比例约 0.56 黄金比例留白。步长取值经 7 类窗口 × 多种难度容量模拟验证：1.45 在紧凑窗口（1024x768 等）下槽位容量仍满足 0.80 覆盖率，且碎片含凸头（最大 1.70 倍基础格）后仍保持明显视觉间隙，杜绝洗牌后碎片视觉相贴。
- **原图逻辑邻居约束分配（方案B）**：初始洗牌（`onLoad` / `resetCurrentGame`）、扫把整理（`organizeTray`）与读档重散落（`_applyBoardState`）统一复用 `_buildScatterAssignment` 贪心约束分配——任一块碎片绝不落入与其原图 4-连通邻居"空间相邻"（中心距 < `tabletopScatterSpatialNeighborRatio = 1.65 · max(pieceSize)`）的散落槽位，从源头杜绝"洗牌后原图相邻碎片看起来已吸附/拼好"的观感。槽位紧缺时回退到"冲突最少"槽位（效果单调不差于完全随机），并完整保留洗牌随机性与同 seed 确定性。
