...
> **文档版本**：v1.0  
> **创建日期**：2026-09-04  
> **基线更新**：2026-09-14（与代码同步：主标签 14→17，grid 3×5=15→3×6=18，taxonomy v3.0.0→v3.2.0 @2026-09-07）  
...
lib/data/constants/puzzle_tags.dart         studio/taxonomy.py                         studio/static/js/taxonomy.js
  ├─ 17 个强类型常量 (kTagLandscapes)         ├─ MAIN_TAGS (17主类)                      ├─ window.TAXONOMY 静态对象
...
- **演进规划**：未来 AI 打标将专门接入 `tag_patterns` 细分词库作为实体候选集，识别出具体实体后再通过 SSOT 的 `normalize_tag()` 算法自动映射到 17 主 Tag 上。因此本次重构将其明确隔离开，避免大标签对 AI 识别造成干扰。
...
1. **数据健全性校验**：
   ...
   - 校验 `tag_patterns` 中包含所有 17 个主标签的 Key；
