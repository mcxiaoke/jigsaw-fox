# 拼图内容打包工具设计方案（Content Packaging Studio）v1.0

> **状态**：方案设计（待实现，不改动现有业务代码）
> **日期**：2026-08-28
> **关联规范**：`docs/puzzle-content-storage-and-expansion-design.md`（v3 存储与扩展）、`docs/jigsaw-image-tagging-specification.md`（25 Tag 单主标签体系）
> **目标用户**：内容运营 / 策展（本地一次性打包）
> **运行形态**：按需启动的本地 `Python Server + 单页 HTML` 工具（`python scripts/content_studio.py` 拉起浏览器即用，关掉即停）
...
**技术选型**：`Python 3.10+ http.server`（标准库）+ 纯原生 `HTML/CSS/JS` 单页，无 `npm` / 无框架。

- 启动：`python scripts/content_studio.py --port 5173 --open`  自动打开 `http://127.0.0.1:5173`。
...
```json
{
  ...
  "generator": "qwen3-vl:8b jigsaw-tag-v1.0",
  "tagVocab": "jigsaw-image-tagging-spec v1.0 (25 tags, single primary)",
...
| `tag` | AI 初标 | 25 枚举之一（`Animals/Pets/Nature/Landscapes/Flowers/Ocean/Birds/Travel/Cities/Architecture/Landmarks/Food/Art/Fantasy/Space/Transportation/People/Sports/Seasons/Holidays/Vintage/Abstract/Cozy/Cartoon/Others`） |
| `confidence` | AI | `0.00-1.00`，`<0.75` 或 `Others` 自动 `reviewRequired=true`（对齐 tagging-spec §11） |
| `correctedTag` | 人工 | 本工具写入；非空时以它为准，空则以 `tag` 为准 |

> **单主标签约束**：每图仅 1 个 `Primary Tag`。`tags.json -> main.json` 时映射为 `tags: ["Pets"]` 单元素数组，兼容 `PuzzleLevelItem.tags: List<String>` 多标签模型（未来可扩展多标签，本期保持单标签以保证 `filterByTag` 命中率）。
...
- **单图校正**：点击图 -> 右侧抽屉 25 个 Tag 单选（中文+英文对照，见 tagging-spec 表），`confidence` 只读展示。
...
| 步骤 | 内容 | 产出 |
|---|---|---|
| 1 | 冻结 `tagVocab` 25 枚举，前端常量 `TAGS_25` | `tool/content_studio/tags.js` |
| 2 | Python Server `scripts/content_studio.py`（scan/thumb/tags/export） | 可运行服务 |
| 3 | 单页 HTML（网格+单选+批量+筛选+保存） | `tool/content_studio/index.html` |
| 4 | 导出：main / events / daily 联调，`X:` 盘实测 | `main.json` 自检通过 |
| 5 | 打磨：拖拽排序、manifest bump、WebP 可选 | v1.0 发布 |

---

## 10. 附录：25 Tag 前台展示映射（与 tagging-spec 一致，供前端 Tab 用）

`Animals/ Pets/ Nature/ Landscapes/ Flowers/ Ocean/ Birds/ Travel/ Cities/ Architecture/ Landmarks/ Food/ Art/ Fantasy/ Space/ Transportation/ People/ Sports/ Seasons/ Holidays/ Vintage/ Abstract/ Cozy/ Cartoon/ Others`
