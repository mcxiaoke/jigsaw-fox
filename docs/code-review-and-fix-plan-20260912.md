...
> 修订：v10 — P2-2（图片失败重试）与 P2-3（下载进度 2 秒节流）极简落地完成（2026-09-12 13:41 GMT+8）
> 历史：v9 复审修正（2026-09-12 13:09 GMT+8）；v8 实施完毕（阶段一~三，见 §8 实施勘误）；v7 补入架构债务溯源（§7）；v6 并入第三轮审查
> 状态：全量实施并测试通过（P2-2 / P2-3 已落地，全量单测 348 项全绿）
...
- ~~**P2-2** 图片失败重试入口、**P2-3** 下载进度局部刷新~~ —— **v10 已按极简方案落地**：源头 2 秒进度节流 + 图片失败点击重试与再进自愈，单测增至 348 项全绿；
