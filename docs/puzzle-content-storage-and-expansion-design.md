...
> **创建日期**：2026-08-24
>
> **⚠️ 实现状态说明**：本文档为**前瞻性规划方案**，其中部分设计与当前已落地实现存在差异，实施前需与现状对齐：
> - **当前已实现**：`custom_puzzles/` 目录下**扁平存储** `custom_*.png`（未采用本文的 `items/ugc_xxx/` + `meta.json` 独立沙盒包结构）；每日挑战为**内置 30 天本地 Bing/Unsplash 数据集**（未接云端 Manifest）；无 DLC 扩展包系统；首页为「分类筛选胶囊 + 双列瀑布流」而非本文的`精选置顶 + 主题可折叠手风琴`。
> - 以下章节中的目录结构、`meta.json`、云端 GitHub Manifest 与 `.jpk` 扩展包均为**待评审的未来演进方向**，不代表当前运行代码。
