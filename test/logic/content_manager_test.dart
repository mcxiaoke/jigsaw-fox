...
void main() {
  ...
  setUp(() {
    sandboxDir = Directory(p.join(Directory.current.path, 'temp', 'test_content_sandbox'));
  ...
  group('CanonicalId Specification Tests', () {
    test('Correctly formats canonical IDs across modules', () {
      ...
      expect(CanonicalId.forEvent('cyberpunk_2026', '01_rain.jpg'), equals('event:cyberpunk_2026:01_rain'));
      expect(CanonicalId.forPack('world_art', 'mona_lisa.png'), equals('pack:world_art:mona_lisa'));
      expect(CanonicalId.forUgc('1787548651000.jpg'), equals('ugc:1787548651000'));
    });
  ...
  group('ContentManager End-to-End Tests with Real Test Server', () {
    test('1. Full sync, Main levels and Multi-tag filtering', () async {
      ...
      final downloadedLevel101 = await manager.ensureMainLevelDownloaded(level101);
      expect(downloadedLevel101.isLocalFile, isTrue);
      expect(File(downloadedLevel101.imagePathOrUrl).existsSync(), isTrue);
    });

    test('2. Daily Challenge monthly Zip download and Time-lock validation', () async {
      final manager = ContentManager(
        bootstrapUrls: ['$testServerBase/manifest.json'],
        appSupportDir: supportDir,
        appDocumentsDir: documentsDir,
      ...
      // 验证 8 月份每日关卡 (共 31 天)
      final augustLevels = manager.getDailyLevelsForMonth('202608', overrideToday: testToday);
      expect(augustLevels.length, equals(31));

      // 验证过去和今天的日期：未加锁 (isTimeLocked == false)
      final levelAug01 = augustLevels.firstWhere((l) => l.dailyDate == '20260801');
      expect(levelAug01.isTimeLocked, isFalse);
      expect(levelAug01.id, equals('daily:20260801'));

      final levelAug27 = augustLevels.firstWhere((l) => l.dailyDate == '20260827');
      expect(levelAug27.isTimeLocked, isFalse);

      // 验证未来日期：加锁 (isTimeLocked == true)
      final levelAug28 = augustLevels.firstWhere((l) => l.dailyDate == '20260828');
      expect(levelAug28.isTimeLocked, isTrue);

      final levelAug31 = augustLevels.firstWhere((l) => l.dailyDate == '20260831');
    ...
    test('3. Events Zip and Array modes, and Auto-GC for disabled events', () async {
      // 预设：在本地创建一个属于 disabled 活动的沙盒目录
      final disabledEventDir = Directory(p.join(documentsDir, 'events', 'expired_cleanup_test'));
      disabledEventDir.createSync(recursive: true);
      File(p.join(disabledEventDir.path, 'garbage.tmp')).writeAsStringSync('old cache');
      ...
      expect(visibleEvents.any((e) => e.id == 'expired_cleanup_test'), isFalse);
      ...
      // 验证 Zip 模式活动下载与关卡映射
      final cyberpunkEvent = visibleEvents.firstWhere((e) => e.id == 'cyberpunk_2026');
      ...
      // 验证 Array 模式活动关卡映射
      final animalEvent = visibleEvents.firstWhere((e) => e.id == 'cute_animals_party');
    ...
    test('4. Robustness: Fallback to backup bootstrap URL on primary failure', () async {
      final manager = ContentManager(
        bootstrapUrls: [
    ...
    test('5. Robustness: Offline cache restoration when totally disconnected', () async {
      // 步骤 1：先在线同步一次，产生本地缓存
    ...
    test('6. Daily Challenge: Tolerates dirty overflow dates and truncated image counts', () async {
      ...
      for (var d = 1; d <= 31; d++) {
        File(p.join(febDir.path, '202602${d.toString().padLeft(2, '0')}.jpg')).writeAsStringSync('fake image');
      }

      // 获取 2 月份关卡，应该严格自动过滤掉 29, 30, 31 号，只保留 28 天
      final febLevels = dailyPipeline.getLevelsForMonth('202602', overrideToday: DateTime(2026, 3, 1));
      ...
      for (var d = 1; d <= 25; d++) {
        File(p.join(sepDir.path, '202609${d.toString().padLeft(2, '0')}.jpg')).writeAsStringSync('fake image');
      }

      // 获取 9 月份关卡，自适应展示 25 个关卡
      final sepLevels = dailyPipeline.getLevelsForMonth('202609', overrideToday: DateTime(2026, 10, 1));
