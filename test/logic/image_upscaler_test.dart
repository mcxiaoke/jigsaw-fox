...
void main() {
  group('ImageUpscaler Engine Tests', () {
    test('shouldUpscale condition check', () {
      ...
      expect(
        ImageUpscaler.shouldUpscale(width: 600, height: 800),
        isTrue,
      ); // 短边600<=750, 长边800<=1000
      expect(
        ImageUpscaler.shouldUpscale(width: 700, height: 1200),
        isTrue,
      ); // 短边700<=750
      expect(
        ImageUpscaler.shouldUpscale(width: 1000, height: 1000),
        isTrue,
      ); // 长边1000<=1000
      expect(
        ImageUpscaler.shouldUpscale(width: 750, height: 1100),
        isTrue,
      ); // 短边750<=750

      // 都不满足时不放大
      expect(
        ImageUpscaler.shouldUpscale(width: 800, height: 1200),
        isFalse,
      ); // 短边800>750, 长边1200>1000
    ...
    test(
      'applyLumaGatedCAS should suppress noise in flat regions while sharpening edges',
      () {
