...
void main() {
  ...
  group('PackContentPipeline End-to-End Tests', () {
    test(
      '1. Import local pure images ZIP (Zero-metadata self-derivation)',
      () async {
        // 构造纯图片 zip (无任何 json)
        final zipPath = p.join(tempDir.path, 'cat_photos.zip');
        final archive = Archive();
        archive.addFile(
          ArchiveFile('cat_01.png', testPngBytes.length, testPngBytes),
        );
        archive.addFile(
          ArchiveFile('cat_02.png', testPngBytes.length, testPngBytes),
        );
        archive.addFile(
          ArchiveFile('cat_03.png', testPngBytes.length, testPngBytes),
        );
    ...
    test('2. Import creator ZIP with pack.json manifest', () async {
      ...
      final archive = Archive();
      archive.addFile(
        ArchiveFile('cover.png', testPngBytes.length, testPngBytes),
      );
      archive.addFile(
        ArchiveFile('neon_01.png', testPngBytes.length, testPngBytes),
      );
      archive.addFile(
        ArchiveFile('neon_02.png', testPngBytes.length, testPngBytes),
      );
      ...
      archive.addFile(
        ArchiveFile('pack.json', manifestBytes.length, manifestBytes),
      );
    ...
    test(
      '3. Duplicate name ZIPs are isolated with unique physical IDs',
      () async {
        final zipPath1 = p.join(tempDir.path, 'same_name.zip');
        final archive1 = Archive();
        archive1.addFile(
          ArchiveFile('a.png', testPngBytes.length, testPngBytes),
        );
        ...
        archive2.addFile(
          ArchiveFile('b.png', testPngBytes.length, testPngBytes),
        );
    ...
    test(
      '4. Whole pack physical deletion releases disk storage and updates index',
      () async {
        final zipPath = p.join(tempDir.path, 'to_delete.zip');
        final archive = Archive();
        archive.addFile(
          ArchiveFile('del_01.png', testPngBytes.length, testPngBytes),
        );
        archive.addFile(
          ArchiveFile('del_02.png', testPngBytes.length, testPngBytes),
        );
