...
void main() {
  ...
  group('AchievementService Unit Tests', () {
    test(
      'onPuzzleSolved triggers first_win, star_1, and no_hint_win',
      () async {
    ...
    test(
      'speed_10min triggers only when pieces >= 100 and seconds <= 600',
      () async {
        final ach = AchievementService.instance;
        final store = AchievementStore.instance;
    ...
    test(
      'Claiming reward grants coins and prevents duplicate claims',
      () async {
        final ach = AchievementService.instance;
        final store = AchievementStore.instance;
    ...
    test(
      'three_star_count deduplicates by canonicalId (design §8.3)',
      () async {
        final ach = AchievementService.instance;
        final store = AchievementStore.instance;
    ...
    test(
      'playSeconds accumulates via lifecycle increments incl. abandoned games (§8.1)',
      () async {
        final ach = AchievementService.instance;
        final store = AchievementStore.instance;
