...
v4.0 differences from my_comfyui_batch_gen.py (functionally identical pipeline,
only prompt assembly changed):
  * Reads my_prompt_library_v4.json by default.
  * Style now comes from a PER-TAG pool (`lib["style_pools"][tag["style_pool"]]`)
    instead of one global list. v3.0 used a single 8-entry style pool that was
    7/8 illustration, so ~half the output drifted to flat anime/cartoon art.
    Real-world tags use the photographic pool (77% of the quota by weight);
    only Art/Cartoon/Fantasy/Holidays/Space/Abstract use the illustration pool.
  * Adds the per-tag `visual_anchors` modifier (the 20-category landmark list),
    so each image carries concrete puzzle landmarks instead of generic filler.
  * Round-robin scheduling is inherited unchanged.
  * `--reseed N`: classic seed gacha. One prompt, N different seeds, modifiers
    held fixed. Normally the seed is drawn from the same RNG that picks the
    modifiers, so prompt and noise vary together and you cannot tell a bad
    prompt from an unlucky roll. reseed=1 keeps the old behaviour bit-for-bit.
...
def build_jobs(
    ...
    * Subject is selected by ROTATION (`subjects[i % len]`) instead of random draw,
      ...
      a new lighting/viewpoint/style/atmosphere combination per subject).
      Prompt-stable across resume runs because i is deterministic per tag.
    * Lighting / viewpoint / style / atmosphere are drawn from a per-job RNG seeded
      by (tag, index, seed_base), so a resumed job reproduces the exact same prompt.

    `reseed` = classic seed gacha:
      reseed=1 (default) -> the seed is taken from the SAME rng that drew the
        modifiers, so prompt and seed vary together. Backwards compatible: keys
        and seeds are bit-identical to pre-reseed runs.
      reseed=N -> the prompt is assembled once per (tag, index) and reused for N
        jobs; only the noise changes, drawn from a separate rng. This is what
        lets you tell "the prompt is bad" apart from "that roll was unlucky":
        if all N cards are unusable the prompt is the problem, not the seed.
      A side benefit: N consecutive jobs share one prompt, so ComfyUI hits its
        conditioning cache. Measured 12.2s vs 16.2s for identical vs distinct
        prompts, so a reseed run often beats --sec-per-image.

    v4.0 specifics:
    * Style is drawn from `lib["style_pools"][tag["style_pool"]]`, NOT from a
      global list. This is what keeps photograph-style tags photographic.
    * `anchor_count` visual anchors are prepended right after the subject so the
      landmarks stay close to the subject in the prompt and carry more weight.
    ...
    for tag in lib["tags"]:
        ...
        for i in range(count):
            ...
            # v4.0: per-tag visual landmarks. Placed immediately after the
            # subject so the attention of a 4B text encoder stays on the pair.
            anchors = tag.get("visual_anchors") or []
            if anchors and anchor_count > 0:
                for a in rng.sample(anchors, min(anchor_count, len(anchors))):
                    parts.append(f"with {a}")

            parts.append(rng.choice(lib["modifiers"]["lighting"]))
            # v4.0: tags that need spatial depth deny overhead/close framings
            # (see VIEWPOINT_DENY in temp/build_v4_library.py).
            ...
            # v4.1: per-tag lens/optics (wide angle + small aperture where the
            # scene spans real depth). Left out entirely for macro/close-up tags.
            optics = tag.get("optics") or []
            if optics:
                parts.append(rng.choice(optics))

            # v4.0: style is tag-specific. Falls back to the v3.0 global list if
            # an old library is passed, so this script still runs on v3.0 data.
            if pools:
                pool = pools.get(tag.get("style_pool", "photo")) or pools.get("photo")
                if pool:
                    parts.append(rng.choice(pool))
            elif "style" in lib["modifiers"]:
                parts.append(rng.choice(lib["modifiers"]["style"]))
            ...
            for r in range(reseed):
                if reseed > 1:
                    # Pure seed gacha: the prompt above is assembled once and
                    # reused, only the noise changes. The seed comes from a
                    # SEPARATE rng so the modifier draw is left untouched --
                    ...
                    # reseed=1 must reproduce the historical key AND the
                    # historical seed sequence, or an existing _progress.json
                    # from a pre-reseed run stops matching.
...
def parse_args(argv=None):
    ...
    p.add_argument(
        "--library",
        default=str(Path(__file__).with_name("my_prompt_library_v4.json")),
        help="prompt library JSON (default: my_prompt_library_v4.json)",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=1,
        help="how many per-tag visual landmarks to inject into each prompt "
        "(0 disables; 2 makes landmarks stronger but dilutes the "
        "quality suffix, because the text encoder is only 4B)",
    )
    p.add_argument(
        "--reseed",
        type=int,
        ...
        "total = subjects * rounds * reseed. Use it to separate a "
    ...
    p.add_argument(
        "--list-tags", action="store_true", help="list tags and quotas then exit"
    )
...
def main(argv=None) -> int:
    ...
    if args.list_tags:
        total = 0
        print(f"{'tag':<16}{'tier':<6}{'pool':<8}{'quot':>6}{'anch':>6}   zh")
        for t in lib["tags"]:
            q = int(t.get("quota", 0))
            total += q
            print(
                f"{t['id']:<16}{t.get('tier', '-'):<6}{t.get('style_pool', '-'):<8}"
                f"{q:>6}{len(t.get('visual_anchors', [])):>6}   {t.get('zh', '')}"
            )
        print(f"{'TOTAL':<16}{'':<6}{'':<8}{total:>6}")
        return 0
    ...
    if args.rounds:
        ...
        print(
            f"[plan] {len(jobs)} jobs, estimated ~{est_hours:.1f}h at {args.sec_per_image}s/img"
        )
    if args.reseed > 1:
        print(
    ...
    # Style-pool split: the number that decides how much of the run is
    # photographic vs illustrated. v3.0 was ~87% illustration and that showed.
