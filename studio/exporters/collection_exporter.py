...
class CollectionExporter(BaseExporter):
    ...
    def execute(self) -> ExportResult:
        ...
        if selected_paths and isinstance(selected_paths, list) and len(selected_paths) > 0:
            ...
            self.log(f"已按指定范围载入 {len(images)} 张待导出图片，合集 ID: {col_id}，模式: {output_mode}", "info")
        else:
            images = scan_images(self.src_p)
            self.log(f"扫描源目录获得 {len(images)} 张图片，合集 ID: {col_id}，模式: {output_mode}", "info")
        ...
        # 重复图片校验拦截：严禁同批次包含重复素材 (防止合集关卡重复)，检测到重复直接报错中止！
        ...
        if dup_groups:
            ...
            err_msg = (
                ...
                f"为避免合集关卡内容重复，请先在素材库中清理或替换重复文件后再执行导出。\n"
                + "\n".join(detail_lines)
            )
