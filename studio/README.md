...
2. 选择要导出的业务模式：
   - **主线关卡 (Main)**：按勾选顺序或升序自增生成主线关卡，自增发布版本号并更新 `main.json`；
   - **月度日历 (Daily)**：选择目标月份（如 `202609`），自动制作日历包 `202609.zip` 并增量更新 `daily.json`；
   - **限时活动 (Event)**：指定活动标识（如 `autumn_festival`）与展示名称，打包活动专属资源；
   - **精选合集 (Collection)**：配置合集 ID 与标题，打包合集归档；
