...
class ManifestRouter {
  ...
  Future<RootManifest> resolveManifest({bool forceRefresh = false}) async {
    if (!forceRefresh && _cachedManifest != null) {
      AppLogger.manifest.fine(
        'resolveManifest cache hit version=${_cachedManifest!.schemaVersion}',
      );
      return _cachedManifest!;
    }

    AppLogger.manifest.info(
      'resolveManifest start forceRefresh=$forceRefresh urls=${bootstrapUrls.length}',
    );
    // 1. 尝试从主备 CDN URL 列表轮询拉取
    for (final url in bootstrapUrls) {
      try {
        AppLogger.manifest.fine(
          'Try fetch manifest ${AppLogger.sanitizeUrl(url)}',
        );
        final json = await _httpClient.fetchJson(
          url,
        ...
        if (json is Map<String, dynamic>) {
          ...
          AppLogger.manifest.warning(
            'Manifest fetch unexpected type ${json.runtimeType} from ${AppLogger.sanitizeUrl(url)}',
          );
        }
      } catch (e, st) {
        AppLogger.manifest.warning(
          'Manifest fetch failed ${AppLogger.sanitizeUrl(url)}',
          e,
          st,
        );
    ...
    // 2. 网络全部失败，回退到本地磁盘缓存
    AppLogger.manifest.warning(
      'All bootstrapUrls failed, fallback to disk cache',
    );
    final diskManifest = await _loadFromDiskCache();
    if (diskManifest != null) {
      AppLogger.manifest.info(
        'Disk cache manifest restored version=${diskManifest.schemaVersion}',
      );
  ...
  Future<void> _saveToDiskCache(Map<String, dynamic> json) async {
    try {
      ...
      AppLogger.manifest.fine(
        'Manifest saved to disk ${AppLogger.sanitizePath(cacheFilePath)}',
      );
    } catch (e, st) {
      AppLogger.manifest.warning(
        'Failed to save manifest to disk ${AppLogger.sanitizePath(cacheFilePath)}',
        e,
        st,
      );
    }
  ...
  Future<RootManifest?> _loadFromDiskCache() async {
    try {
      ...
      if (file.existsSync()) {
        ...
        AppLogger.manifest.fine(
          'Disk cache file not found ${AppLogger.sanitizePath(cacheFilePath)}',
        );
      }
    } catch (e, st) {
